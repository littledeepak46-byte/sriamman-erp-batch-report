      ...
      <table style={{ borderCollapse: "collapse", width: "100%", tableLayout: "fixed" }}>
          ...
          <tr style={{ height: "16pt" }}>
            {recPerM3.map((v, i) => (
              <td key={i} style={{ ...tc, width: cols[i].w }}>
                {v === 0 ? (cols[i].dec ? "" : "0") : cols[i].dec ? v.toFixed(cols[i].dec) : fv(v)}
              </td>
            ))}
          </tr>
        </thead>
        ...
        <tbody>
          ...
          {rows.map((row, bIdx) => [
            ...
            <tr key={`a${bIdx}`} style={{ height: "16pt" }}>
              {cols.map((c, i) => {
                ...
                const disp  = act === 0 ? (c.dec ? "" : "0") : c.dec ? act.toFixed(c.dec) : fv(act);
                return (
                  <td key={c.key} style={{ ...tc, width: c.w,
            ...
            <tr key={`c${bIdx}`} style={{ height: "16pt" }}>
              {cols.map((c) => (
                <td key={c.key} style={{ ...tc, width: c.w, color: "#555" }}>
                  {AGG_KEYS.has(c.key) ? "0.00" : c.dec ? "" : "0"}
                </td>
              ))}
            </tr>,
          ...
          <tr style={{ height: "16pt" }}>
            {cols.map((c, i) => (
              <td key={c.key} style={{ ...tc, width: c.w, fontWeight: "bold" }}>
                {setTotals[i] === 0 ? (c.dec ? "" : "0") : c.dec ? setTotals[i].toFixed(c.dec) : fv(setTotals[i])}
              </td>
            ))}
          </tr>
          ...
          <tr style={{ height: "16pt" }}>
            {cols.map((c, i) => (
              <td key={c.key} style={{ ...tc, width: c.w, fontWeight: "bold" }}>
                {actTotals[i] === 0 ? (c.dec ? "" : "0") : c.dec ? actTotals[i].toFixed(c.dec) : fv(actTotals[i])}
              </td>
            ))}
          </tr>
